...
const SeeTodo = ({ appSwitch }) => {
    ...
    const sendInnerHtml=(html)=>{
        ...
        console.log(html.target.className);
    }
        ...
        {inData.map((data, index) => (
            <div className='wrapper_div' key={index}>
                <div className="forCss_div">
                    <p onClick={(html) => { clickFnk(data);sendInnerHtml(html)}} className='text_p'>{data.data().todo} - {String(data.data().timeStamp.toDate()).split('GMT+0900 (日本標準時)')[0]}</p>
                </div>
            </div>
        ))}
        </div>
    ...
    const selectHtml=[
        ...
        <button onClick={()=>setSwitcher('done')}>実行済みにする</button>
